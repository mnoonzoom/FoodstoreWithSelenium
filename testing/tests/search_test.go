...
func Test_QuickBite_Search_Filters_Menu(t *testing.T) {
	...
	if len(newTitles) > 0 {
		...
		if !strings.Contains(strings.ToLower(t0), strings.ToLower(query)) {
			t.Logf("QuickBite: warning: first result title %q does not contain query %q (filter might be fuzzy)", t0, query)
		}
	}
}
